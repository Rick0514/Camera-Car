...
public class BleManager {
    ...
    public void write_b(BleDevice bleDevice,
                        String services,
                        String characteristic,
                        String msg,
                        BleWriteCallback bleWriteCallback) {
        write_b(bleDevice, services, characteristic, msg, true, true, 0, bleWriteCallback);
    }
    ...
    public void write_b(BleDevice bleDevice,
        ...
        if (data.getBytes().length > 20 && !split) {
            BleLog.w("Be careful: data's length beyond 20! Ensure MTU higher than 23, or use spilt write!");
        }
        ...
        if (bleBluetooth == null) {
            ...
            if (split && data.getBytes().length > getSplitWriteNum()) {
                callback.onWriteFailure(new OtherException("Too long string!"));
            } else {
                bleBluetooth.newBleConnector()
                        .withUUIDString(uuid_service, uuid_write)
                        .writeCharacteristic(data, callback, uuid_write);
